...
if json_flag "$SITE_JSON" convertImagesToWebp; then
  CONVERT_IMAGES_TO_WEBP="true"
fi

LOGO_SRC="/logo.png"
if [ "$CONVERT_IMAGES_TO_WEBP" = "true" ]; then
  LOGO_SRC="/logo.webp"
fi
...
build_hero() {
  ...
  local img_tag
  if [ "$hero_lazy" = "1" ]; then
    img_tag="<img src=\"$(image_placeholder_src "/img/pages/$hero_img")\" data-src=\"/img/pages/$hero_img\" loading=\"lazy\" alt=\"${hero_line1}\">"
  else
...
render_partial() {
  ...
  case "$name" in
    ...
    page-image)
      ...
      local page_img_ext="jpg"
      if [ "$CONVERT_IMAGES_TO_WEBP" = "true" ]; then
        page_img_ext="webp"
      fi
      if [ -f "$OUTPUT_DIR/img/pages/${pname}.${page_img_ext}" ]; then
        local tpl
        tpl=$(<"$TEMPLATE_DIR/partials/page-image.html")
        local ptitle=$(json_val "$page_json" title)
        local pshort="${ptitle%% |*}"
        local page_img_src="${pname}.${page_img_ext}"
        local page_blur_src
        page_blur_src=$(image_placeholder_src "$page_img_src")
        render_template "$tpl" \
          "page_blur_src" "$page_blur_src" \
          "page_img_src"  "$page_img_src" \
          "page_heading"  "$pshort"
      fi
      ;;
    parts)
...
build_product_card_item() {
  ...
  html+="<img src=\"/img/products/$(image_placeholder_src "$img")\" data-src=\"/img/products/${img}\" loading=\"lazy\" alt=\"${L_BRAND} ${name}\" title=\"${L_BRAND} ${name}\">"
  html+="<h3>${name}</h3></a>"
  html+="<p>${short_desc}</p>"
...
build_products() {
  ...
  for pj in "$SETTINGS_DIR"/products/*.json; do
    ...
    local blur
    blur=$(image_placeholder_src "$img")
...
build_sw() {
  ...
  for f in "$OUTPUT_DIR"/img/products/$img_glob; do
  ...
  for f in "$OUTPUT_DIR"/img/pages/$img_glob; do
